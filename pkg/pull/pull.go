...
import (
	...
	"github.com/konghui/docker-tool/mainifest"
	"github.com/konghui/docker-tool/pkg/utils"
)
...
func (i *ImageSource) GetBlobInfos(manifestByte []byte, manifestType string) ([]types.BlobInfo, error) {
	...
	manifestInfo, digests, err := mainifest.ManifestHandler(manifestByte, manifestType)
	if err != nil {
		return nil, err
	}
...
func (i *ImageSource) GetABlob(blobInfo types.BlobInfo) (io.ReadCloser, int64, error) {
	return i.source.GetBlob(i.ctx, types.BlobInfo{Digest: blobInfo.Digest, Size: -1}, none.NoCache)
}
